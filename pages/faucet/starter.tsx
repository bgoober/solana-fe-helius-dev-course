...
const Starter = () => {
  // allocate state to hold tx sig
  const [txSig, setTxSig] = React.useState<string>("");

  // add connection objects and pubkey from wallet
  const { connection } = useConnection();
  const { publicKey, sendTransaction } = useWallet();

  // function to fund the wallet with devnet SOL from the faucet
  const fundWallet = async (event: { preventDefault: () => void }) => {
    event.preventDefault();

    // if user is not connected or no network connection, error
